...
ax3 = plt.subplot2grid((4, 8), (0, 0),colspan=4,rowspan=4)
# ax2 = plt.subplot2grid((4, 8), (0, 2),colspan=2,rowspan=2)
# ax1 = plt.subplot2grid((4, 8), (2, 1),colspan=2,rowspan=2)
# ax4 = plt.subplot2grid((4, 8), (2, 2),colspan=2,rowspan=2)
ax5 = plt.subplot2grid((4, 8), (0, 4),colspan=4,rowspan=4)

axins = zoomed_inset_axes(ax5, 2.5, loc=1) # zoom-factor: 2.5, location: upper-left
axins2 = zoomed_inset_axes(ax5, 2.5, loc=4) # zoom-factor: 2.5, location: upper-left
...
ax5.plot(bx,by,'--k',linewidth=0.5)
axins.plot(bx,by,'--k',linewidth=0.5)
axins2.plot(bx,by,'--k',linewidth=0.5)
# ax6.plot(bx,by,'--k',linewidth=0.5)
...
for i in range(len(parX)):
    ...
    circ = plt.Circle((parX[i],parY[i]), rotorDiameter[i]/2.,facecolor=robin,edgecolor=robin,alpha=0.5)
    axins2.add_patch(circ)


# circ = plt.Circle((parX[0],parY[0]), rotorDiameter[0]/2.,facecolor=None,edgecolor=royal,alpha=0.5=2)
# axins.add_patch(circ)
axins.plot(parX[0],parY[0],'o',markerfacecolor=None,markeredgecolor=royal,alpha=0.3,mew=1.5,markersize=7)
# circ = plt.Circle((2244.26,1079.93), rotorDiameter[0]/2.,facecolor='None',edgecolor=tuscan)
# axins.add_patch(circ)

axins.plot(2012.73,1516.54,'o',color=royal,alpha=0.3,markersize=3)

axins.plot(np.array([2012.73,parX[0]]),np.array([1516.54,parY[0]]),'-',color=royal,linewidth=2.5,alpha=0.3)
axins.text(1820.,1425.,'s',color=royal,size=8,alpha=0.75)
# axins.text(1700.,800.,'s=0',color=tuscan,size=10)
# axins.arrow(2020.,950.,100.,60.,head_width=50.,head_length=50,color=tuscan)

axins.set_xlim(1400., 2400.) # apply the x-limits
axins.set_ylim(1000., 2000.) # apply the y-limits

axins2.set_xlim(-1000., 500.) # apply the x-limits
axins2.set_ylim(-1100., 400.) # apply the y-limits

# axins2.plot(np.array([-700.67397107,-700.67397107]),np.array([-1026.50430634,16.19155175]),linewidth=2.,color=royal,alpha=0.5)
# axins2.plot(np.array([-700.67397107,-446.47371909]),np.array([-1026.50430634,-1026.50430634]),linewidth=2.,color=royal,alpha=0.5)

n = 50.

m = 2.368488665241571
step = 360.
axins2.plot(np.array([-531.21201715,-264.03607095]),np.array([-782.04411712,-894.84851856]),linewidth=1.,color=royal,alpha=0.5)
axins2.plot(np.array([-531.21201715,-531.21201715+step]),np.array([-782.04411712,-782.04411712+step*m]),linewidth=1.,color=royal,alpha=0.5)
axins2.text((-531.21201715-264.03607095)/2.-50.,-980.,'dx',color=royal,alpha=0.75,size=8,horizontalalignment='center')
axins2.text(-300.,(-782.04411712-782.04411712+step*m)/2.,'dy',color=royal,alpha=0.75,size=8,verticalalignment='center')



axins2.plot(np.array([-531.21201715+step,-413.10512002]),np.array([-782.04411712+step*m,174.41718261]),linewidth=1.,color=royal,alpha=0.5)
axins2.text(-300.,180.,'b',color=royal,alpha=0.75,size=8,verticalalignment='center')

axins2.plot(np.array([-531.21201715,-531.21201715]),np.array([-782.04411712,200.]),linewidth=1.,linestyle='--',color=royal,alpha=0.5)
arc = matplotlib.patches.Arc((-531.21201715,-782.04411712),1300.,1300.,theta1=68.,theta2=90.,linestyle='-',color=royal,alpha=0.5)
axins2.add_patch(arc)
axins2.text(-450.,-100.,r'$\theta$',color=royal,alpha=0.75,size=8)

step2 = 100.
m2 = -1./m
import math
t = math.atan2(step,step*m)
axins2.plot(np.array([-531.21201715+step2*np.sin(t),-531.21201715+step2*np.sin(t)+step2*np.cos(t)]),np.array([-782.04411712+step2*np.cos(t),-782.04411712+step2*np.cos(t)-step2*np.sin(t)]),linewidth=1.,color=royal,alpha=0.5)
axins2.plot(np.array([-531.21201715+step2*np.cos(t),-531.21201715+step2*np.cos(t)+step2*np.sin(t)]),np.array([-782.04411712-step2*np.sin(t),-782.04411712+step2*np.cos(t)-step2*np.sin(t)]),linewidth=1.,color=royal,alpha=0.5)


# axins2.plot(np.array([-700.67397107-n-100.,-700.67397107-100.]),np.array([-1026.50430634,-1026.50430634]),linewidth=1.,color=royal,alpha=0.5)
# axins2.plot(np.array([-700.67397107-n-100.,-700.67397107-100.]),np.array([16.19155175,16.19155175]),linewidth=1.,color=royal,alpha=0.5)
# axins2.plot(np.array([-700.67397107-100.-n/2.,-700.67397107-100.-n/2.]),np.array([-1026.50430634,16.19155175]),linewidth=1.,color=royal,alpha=0.5)
#
# axins2.plot(np.array([-700.67397107,-700.67397107]),np.array([-1026.50430634-n-100.,-1026.50430634-100.]),linewidth=1.,color=royal,alpha=0.5)
# axins2.plot(np.array([-446.47371909,-446.47371909]),np.array([-1026.50430634-n-100.,-1026.50430634-100.]),linewidth=1.,color=royal,alpha=0.5)
# axins2.plot(np.array([-700.67397107,-446.47371909]),np.array([-1026.50430634-n/2.-100.,-1026.50430634-n/2.-100.]),linewidth=1.,color=royal,alpha=0.5)
#
# axins2.plot(np.array([-446.47371909,61.92678487]),np.array([-1026.50430634,-1026.50430634]),'--',linewidth=1.,color=royal,alpha=0.5)
# axins2.plot(np.array([-700.67397107,-336.35668994]),np.array([-1026.50430634,16.19155175]),'--',linewidth=1.,color=royal,alpha=0.5)
# axins2.plot(np.array([-700.67397107,61.92678487]),np.array([-1026.50430634,-1190.33375143]),'--',linewidth=1.,color=royal,alpha=0.5)
#
# arc = matplotlib.patches.Arc((-700.67397107,-1026.50430634),1000.,1000.,theta1=70.,theta2=90.,linestyle='-',color=royal,alpha=0.5)
# axins2.add_patch(arc)
# arc = matplotlib.patches.Arc((-700.67397107,-1026.50430634),1000.,1000.,theta1=-12.,theta2=0.,linestyle='-',color=royal,alpha=0.5)
# axins2.add_patch(arc)
#
# axins2.text(-650.,-500.,r'$\bf{\theta}$',color=royal,alpha=0.5,size=10)
# axins2.text(-150,-1125.,r'$\bf{\phi}$',color=royal,alpha=0.5,size=10)

from mpl_toolkits.axes_grid1.inset_locator import mark_inset
mark_inset(ax5, axins, loc1=3, loc2=4, fc="none", ec="0.5")
mark_inset(ax5, axins2, loc1=1, loc2=2, fc="none", ec="0.5")
...
# drawCirc(axins2,300.,-100.,-500.,120.,225.,color_=tuscan)
# axins2.text(-50,-750.,r'$\bf{\alpha}$',color=tuscan,size=10)
...
ax3.text(-2500.,2700.,'a',fontsize=11,color='black')
ax5.text(-2500.,2700.,'b',fontsize=11,color='black')
ax5.text(750.,4500.,'c',fontsize=11,color='black')
ax5.text(-500.,-4000.,'d',fontsize=11,color='black')
